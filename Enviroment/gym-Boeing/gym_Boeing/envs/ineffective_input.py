"""
This module contains 3 different modes for the B matrix.
 The B matrix is now 0.5 its original value, or the throttle is ineffective and stuck at zero. 
...
ineffective = np.array([[1,0],[0,0]])
B_inef      = np.matmul(B, ineffective)

class FailureMode6(gym.Env):
    """
    Gym enviroment containing the normal modeo of operation and on with an ineffective B matrix and a non working throttle
    """
    ...
    def __init__(self):
        ...
        """        
        self.done = False
        self.flight = Flight(failure_modes=[[A,B_err,C,D], [A,B_inef,C,D]])
