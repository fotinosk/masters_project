...
class TD3_Trainer():
    ...
    def update(self, batch_size, deterministic, eval_noise_scale, reward_scale=10., gamma=0.9, soft_tau=1e-2):
        ...
        # reward = reward_scale * (reward - reward.mean(dim=0)) / (reward.std(dim=0) + 1e-6) # normalize with batch
        # mean and std; plus a small number to prevent numerical problem
...
explore_steps = 100  # for random action sampling in the beginning of training
update_itr = 1
hidden_dim = 400  # original 512
            ...
            for step in tqdm(range(max_steps)):
                ...
                if len(replay_buffer) > batch_size:
                    for i in range(update_itr):
                        _ = td3_trainer.update(batch_size, deterministic=DETERMINISTIC,
                                               eval_noise_scale=eval_noise_scale, reward_scale=reward_scale)
