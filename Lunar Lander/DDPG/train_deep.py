...
if __name__ == "__main__":
    ...
    while timestep <= timesteps:
        ...
        while True:
            ...
            if done:
                print(f"Overall: {timestep} | Timestep: {timestep-last_timestep} | Episode Reward: {epoch_return:.{0}f} | Time taken: {time.time()- t0:.{2}f}sec")
        ...
        if timestep >= 10000 * t:
            ...
            while True:
                runs += 1
                state = torch.Tensor([env.reset(mode = runs % modes)]).to(device)
                test_reward = 0
                agent.set_eval()
                while True:
