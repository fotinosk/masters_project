...
if __name__ == "__main__":
    ...
    augment = Augment(state_size=env.observation_space.shape[0], action_size=env.action_space.shape[0], memory_size=40)
    ...
    while timestep <= timesteps:
        ...
        while True:
            ...
            if done:
                if reward > 0:
                    print(colored(f"Timestep: {timestep-last_timestep} | Episode Reward: {epoch_return:.{0}f} | Time taken: {time.time()- t0:.{2}f}sec", "green"))
                else:
                    print(colored(f"Timestep: {timestep-last_timestep} | Episode Reward: {epoch_return:.{0}f} | Time taken: {time.time()- t0:.{2}f}sec", "red"))
        ...
        if timestep >= 10000 * t:
            ...
            while True:
                ...
                elif runs == 10 and np.mean(test_rewards) > 100:
