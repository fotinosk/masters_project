"""
2 different mass modes, default = 4.1666 and alt = 8
"""
...
class LunarLander(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        if self.game_over or abs(state[0]) >= 1.0 or self.timestep > 2500:
            done = True
            reward = -100
        ...
        observable_state = np.array(state, dtype=np.float32)[[0,1,4,5,6,7]] # exclude vert and horiz velocity
        return observable_state, reward, done, {'success': reward == 100}
...
class MassModeTrainPO(LunarLander):
    continuous = True
