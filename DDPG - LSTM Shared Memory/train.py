...
def train(rank, params, shared_model, optimizer):
    ...
    model = DDPG(env.observation_space.shape[0], env.action_space) # creating the model from the ActorCritic class
    state = env.reset() 
    ...
    while True: # repeat
        ...
        if done: # if it is the first iteration of the while loop or if the game was just done, then:
            cx = Variable(torch.zeros(1, 400)) # the cell states of the LSTM are reinitialized to zero
            hx = Variable(torch.zeros(1, 400)) # the hidden states of the LSTM are reinitialized to zero
        else: # else:
