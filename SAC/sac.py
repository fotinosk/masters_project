...
value_net        = ValueNetwork(num_inputs, hidden_dim, activation=F.relu).to(device)
target_value_net = ValueNetwork(num_inputs, hidden_dim, activation=F.relu).to(device)

soft_q_net1 = SoftQNetwork(num_inputs, action_dim, hidden_dim, activation=F.relu).to(device)
soft_q_net2 = SoftQNetwork(num_inputs, action_dim, hidden_dim, activation=F.relu).to(device)
policy_net = PolicyNetwork(num_inputs, action_dim, hidden_dim, activation=F.relu).to(device)
    ...
    if args.train:
        ...
        for eps in range(max_episodes):
            ...
            for step in range(max_steps):
                state = augment(torch.Tensor([state]).to(device)[0])
                ...
                n_state_aug = augment.mock_augment(torch.tensor(next_state).to(device), state, torch.tensor(action).to(device))

                replay_buffer.push(state, action, reward, n_state_aug, done)
