...
# def get_input():
#     """Gives the DQN the output of the aircraft model"""
...
# TODO: Implement function that discretizes controls and returns actions
n_actions = []

# 1 and 3 are the dimensions of the input tensor, might need to change (or change conv2d to conv1d)
policy_net = DQN(1, 3, n_actions).to(device)
target_net = DQN(1, 3, n_actions).to(device)
...
def optimize_model():
    """
     performs a single step of the optimization. It first samples a batch, concatenates all the tensors
      into a single one, computes Q(st,at) and V(st+1)=max_a{Q(st+1,a)}, and combines them into our loss.
      By defition we set V(s)=0 if s is a terminal state. We also use a target network to compute V(st+1)
      for added stability. The target network has its weights kept frozen most of the time, but is updated
      with the policy network’s weights every so often. This is usually a set number of steps but we shall
      use episodes for simplicity.
    """
