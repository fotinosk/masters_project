...
class Actor(nn.Module):
    def __init__(self, hidden_size, num_inputs, action_space):
        ...
        #LSTM Layer 3
        self.lstm = nn.LSTM(hidden_size[1], hidden_size[2])
    ...
    def forward(self, inputs):
        ...
        # LSTM Layer 3
        x, _ = self.lstm(x.view(x.size(0), self.batch_size, -1))
        x = self.ln3(x)
        x = F.relu(x)
...
class Critic(nn.Module):
    def __init__(self, hidden_size, num_inputs, action_space):
        ...
        # fan_in_uniform_init(self.lstm.weight)
        # fan_in_uniform_init(self.lstm.bias)
