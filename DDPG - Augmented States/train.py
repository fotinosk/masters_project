...
if __name__ == "__main__":
    ...
    trajectory = Trajectory(state_size=3, action_size=env.action_space.shape[0])
    num_inputs = trajectory.num_out()
    ...
    while timestep <= args.timesteps:
        ...
        while True:
            ...
            state = trajectory(state[0])
            action = agent.calc_action(state, ou_noise)
            if action.shape[0] != 2:
                action = action[0].to(device)
            next_state, reward, done, _ = env.step(action.cpu().numpy())
            trajectory.archive([state, action, next_state, reward])
            print(done, _)
            timestep += 1
            epoch_return += reward
            ...
            if len(memory) > args.batch_size:
                ...
                x = trajectory.sample()
                x = torch.stack(x)
                # batch = torch.split(x, [3,2,3,1], dim=-1)
                # value_loss, policy_loss = agent.update_params(batch)
                value_loss, policy_loss = agent.update_params(x)
